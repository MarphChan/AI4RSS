...
def main():
    i18n.init_language()
    render_sidebar_navigation(active="sources")
    ...
    if not sources:
        ...
        current_ids = set(base_df["id"].tolist())
        selected_ids = set(st.session_state.get("sources_selected_ids", []))
        selected_ids = selected_ids.intersection(current_ids)
        st.session_state["sources_selected_ids"] = list(selected_ids)

        draft_df = st.session_state.get("sources_draft_df")
        if (
            draft_df is None
            or set(draft_df.get("id", pd.Series(dtype=str)).tolist()) != current_ids
            or list(draft_df.columns) != list(base_df.columns)
        ):
            st.session_state["sources_draft_df"] = base_df.copy()
            draft_df = st.session_state["sources_draft_df"]

        editor_df = draft_df.copy()
        editor_df.insert(0, "Select", editor_df["id"].isin(selected_ids))
        type_options = []
        for t in ["rss", "web_crawl", "social"]:
            if t not in type_options:
                type_options.append(t)
        for t in sorted(set(editor_df["type"].dropna().astype(str).tolist())):
            if t and t not in type_options:
                type_options.append(t)
        ...
        st.session_state["sources_selected_ids"] = selected_ids

        draft_df = edited_df.drop(columns=["Select"])
        st.session_state["sources_draft_df"] = draft_df

        editable_cols = [c for c in ["enabled", "group", "name", "type", "url", "logo"] if c in base_df.columns]
        is_dirty = not base_df.equals(draft_df)
        if st.button(i18n.get_text("save_changes_button"), type="primary", disabled=not is_dirty):
            base_by_id = base_df.set_index("id")
            draft_by_id = draft_df.set_index("id")
            updates_by_id = {}
            for source_id in base_by_id.index.intersection(draft_by_id.index):
                updates = {}
                for col in editable_cols:
                    base_val = base_by_id.at[source_id, col]
                    draft_val = draft_by_id.at[source_id, col]
                    if pd.isna(base_val) and pd.isna(draft_val):
                        continue
                    if base_val != draft_val:
                        updates[col] = draft_val
                if updates:
                    updates_by_id[source_id] = updates

            source_manager.update_sources_bulk(updates_by_id)
            st.toast(i18n.get_text("changes_saved_toast"))
            if "sources_table" in st.session_state:
                del st.session_state["sources_table"]
            if "sources_draft_df" in st.session_state:
                del st.session_state["sources_draft_df"]
            st.rerun()
        ...
        with col_batch_del:
            if st.button(i18n.get_text("batch_delete_button"), type="primary"):
                ids_to_delete = st.session_state.get("sources_selected_ids", [])
                if ids_to_delete:
                    count = source_manager.delete_sources(ids_to_delete)
                    st.session_state["sources_selected_ids"] = []
                    if "sources_table" in st.session_state:
                        del st.session_state["sources_table"]
                    if "sources_draft_df" in st.session_state:
                        del st.session_state["sources_draft_df"]
